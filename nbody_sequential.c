...
// Function to calculate gravitational forces (sequential brute force algorithm)
void calculate_forces(Body* bodies, int n) {
    ...
    for (int i = 0; i < n; i++) {
        for (int j = i + 1; j < n; j++) {
            ...
            bodies[i].force_z += force_z;
            
            bodies[j].force_x -= force_x;
            bodies[j].force_y -= force_y;
            bodies[j].force_z -= force_z;
        }
    }
}

// Function to update positions and velocities (Euler integration)
void update_bodies(Body* bodies, int n, double dt) {
    for (int i = 0; i < n; i++) {
        ...
        bodies[i].vel_z += (bodies[i].force_z / bodies[i].mass) * dt;
...
// Function to print the status of bodies
