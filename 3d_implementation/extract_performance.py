...
RESULTS_DIR = "results"
PLOTS_DIR = "plots"
DATASETS = ["bodies_10.csv", "bodies_100.csv", "bodies_500.csv", "bodies_1000.csv", "bodies_2000.csv", "bodies_3000.csv", "bodies_5000.csv", "bodies_10000.csv"]
...
def collect_performance_data():
    ...
    SEQ_DIR = os.path.join(RESULTS_DIR, "seq_dataset")
    PAR_DIR = os.path.join(RESULTS_DIR, "par_dataset")
...
def create_plots(metrics_df):
    ...
    seq_times = [metrics_df[metrics_df['dataset'] == d]['seq_time'].iloc[0] for d in datasets]
    
    # Parallel times for 8 processors
    par_times_8 = []
    for dataset in datasets:
        data = metrics_df[(metrics_df['dataset'] == dataset) & (metrics_df['processors'] == 8)]
        if not data.empty:
